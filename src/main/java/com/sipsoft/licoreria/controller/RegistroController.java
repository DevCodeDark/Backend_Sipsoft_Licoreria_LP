...
public class RegistroController {
    ...
    @Transactional
    @Operation(summary = "Registrar un nuevo cliente", description = "Crea un nuevo registro de cliente y devuelve el clienteId y la llaveSecreta generados.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Cliente registrado exitosamente", content = @Content(schema = @Schema(implementation = RegistroCompletoResponseDTO.class))),
    ...
    public ResponseEntity<Object> crear(@RequestBody RegistroDTO dto) {
        ...
        Registro registroGuardado = registroService.guardar(registro);
    ...
    @Operation(summary = "Obtener todos los registros", description = "Devuelve una lista de todos los registros de clientes sin información sensible.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Lista de registros obtenida exitosamente", content = @Content(schema = @Schema(implementation = RegistroResponseDTO.class))),
    ...
    public ResponseEntity<List<RegistroResponseDTO>> obtenerTodos() {
        ...
        List<RegistroResponseDTO> response = registros.stream().map(registro -> new RegistroResponseDTO(
                registro.getIdRegistro(),
                registro.getEmail(),
                registro.getNombre(),
                registro.getApellido(),
                registro.getClienteId(),
                registro.getLlaveSecreta())).toList();
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Registro encontrado exitosamente", content = @Content(schema = @Schema(implementation = RegistroResponseDTO.class))),
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Registro actualizado exitosamente", content = @Content(schema = @Schema(implementation = RegistroResponseDTO.class))),
            @ApiResponse(responseCode = "404", description = "Registro no encontrado"),
